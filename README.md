...
Members without a photo get a unique geometric placeholder at `assets/images/placeholders/{slug}.svg`. These files are generated by `pnpm placeholders` (also run in CI) and are **not** rewritten once they exist. Adding a real `"photo"` path in `members.json` takes precedence automatically.

For alumni, a public LinkedIn profile photo takes priority over the local `"photo"` when one exists. Refresh the cache with `pnpm linkedin-photos` (also run in CI before deploy).
