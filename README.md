...
**Add yourself or edit lab data:** [CONTRIBUTING.md](CONTRIBUTING.md) · [راهنمای فارسی](docs/contributing.fa.md)
...
│   ├── presentations.json
│   ├── projects.json
│   ├── linkedin-photos.json   # generated; do not hand-edit
│   └── github-stats.json      # generated; do not hand-edit
├── assets/                    # favicon and images/ (member photos)
├── src/web/                   # Vite + React app (@dslab/web)
├── scripts/                   # stats, LinkedIn photos, placeholders
├── CONTRIBUTING.md            # how to add yourself and edit content
├── docs/contributing.fa.md    # same guide in Persian
...
## Contributing (add yourself or edit content)

You do **not** need to touch React or CSS to appear on the site. Edit JSON under `data/`, add a photo under `assets/images/` if you have one, and open a pull request.

**Exact steps** (GitHub editor or local clone, field-by-field, photos, alumni, papers, talks): **[CONTRIBUTING.md](CONTRIBUTING.md)** · راهنمای فارسی: **[docs/contributing.fa.md](docs/contributing.fa.md)**.

Quick map:

| Goal | File |
|---|---|
| Add yourself / update your profile / become alumni | `data/members.json` |
| Portrait | `assets/images/<your-slug>.jpg` and set `"photo"` |
| Research thread (Now Building) | `data/current-work.json` |
| Publication | `data/projects.json` |
| Upcoming talk | `data/presentations.json` |

Current members should set `"leadership": "member"`. Names in work, papers and talks must match `name` or `github` in `members.json` (case-insensitive). Do not hand-edit `github-stats.json` or `linkedin-photos.json`.

---

## Member & project data

Vite copies `data/` and `assets/` into the web app on every `dev` and `build`. Step-by-step recipes live in [CONTRIBUTING.md](CONTRIBUTING.md); this section is the schema overview.
...
├── presentations.json   # Upcoming talks; `"presentations": []` when none are scheduled
├── projects.json        # Selected publications, tags, contributors and DOI links
├── linkedin-photos.json # Alumni LinkedIn photo cache (CI; do not hand-edit)
└── github-stats.json    # Aggregate public GitHub activity (CI; do not hand-edit)
```

Photos referenced as `"photo": "assets/images/name.jpg"` go in `assets/images/` (not `placeholders/` or `linkedin/`). Members without a photo get `assets/images/placeholders/{slug}.svg` from `pnpm placeholders` (also run in CI). Those files are **not** rewritten once they exist. A real `"photo"` path takes precedence.

For alumni, a public LinkedIn profile photo takes priority over the local `"photo"` when the cache has one. Refresh with `pnpm linkedin-photos` (also run in CI).

### People (`members.json`)

JSON array. Order in the file is display order within each section. `leadership` controls where someone appears:

| `leadership` | Where they show |
|---|---|
| `director` | Featured director card |
| `lead` | Core leads grid |
| `member` (or omitted) | Members grid — use this to add yourself |
...
`github` is a **username**, not a URL. `degree` may be `bsc`, `msc`, or `phd` (aliases such as `bachelor` / `master` also work). Optional profile fields: `email`, `scholar`, `researchgate`, `scopus`, `dblp`, `homepage`, `years`, `thesis`, `aka`, `affiliation`, `position`, `location`.

For alumni, `role` stays the lab status (`PhD Alumnus`). Current workplace goes in `position` + `affiliation` (+ optional `location`). Set `alumniGroup` or the card will not appear in a group. Full alumni recipe: [CONTRIBUTING.md](CONTRIBUTING.md#move-from-member-to-alumni).

Profile URL: `name` is slugified (`Prof.` / `Dr.` stripped) → `/people/your-name`.

### Current work

Edit `data/current-work.json` to update **Now Building**. JSON array — add, remove or reorder objects.
...
Edit `data/presentations.json` (an object with a `presentations` array). The `member` value is matched against `name` or `github` in `members.json`; the card then reuses that person’s photo, role and research-focus tags.
...
| Doc | What it covers |
|---|---|
| [README.md](README.md) | Lab overview, stack, schema, local build |
| [CONTRIBUTING.md](CONTRIBUTING.md) | Exact steps to add yourself, photos, alumni, papers, talks |
| [docs/contributing.fa.md](docs/contributing.fa.md) | همان راهنما به فارسی |
| [design/SKILL.md](design/SKILL.md) | Visual language (tokens, type, layout) |
| [animation.md](animation.md) | Motion tokens and keyframes |
