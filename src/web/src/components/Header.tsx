...
function isNavActive(href: string, route: Route): boolean {
  ...
  if (href === PATHS.people) {
    return route.name === "people" || route.name === "member";
  }
...
export function Header() {
  ...
  useEffect(() => {
    /*
     * Header height is intentionally controlled by CSS.
     *
     * Do NOT measure .site-header-inner and write the result
     * back into --header-height. Doing so creates a feedback
     * loop when the header itself uses --header-height.
     *
     * The CSS currently defines:
     *   desktop: 80px
     *   mobile: 72px
     */
